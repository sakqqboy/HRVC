...
if (isset($d) && count($d) > 0) {
	foreach ($d as $branchId => $department) : ?>
		...
		?>
			<div class="col-12 multi-select pl-30 pt-5 pb-5">
				<input type="checkbox" <?= $check ?> id='multi-check-<?= $branchId ?>-update' name="department[]" required class="checkbox-md mr-5 multi-check-department-update multiDepartment-<?= $branchId ?>" value="<?= $departmentId ?>" onchange="javascript:departmentMultiTeamUpdateKpi(<?= $branchId ?>)">
				<?= $departmentName ?>
			</div>
<?php
		endforeach;
