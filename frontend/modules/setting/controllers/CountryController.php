...
class CountryController extends Controller
{
    ...
    public function actionCountryPicture()
    {
        ...
        foreach ($country as $c):
            ...
            $c->save(false);
        endforeach;
        $text = "Afghanistan|AF|AFG|4*Albania|AL|ALB|8*
Algeria|DZ|DZA|12*
...
Aland Islands|AX|ALA|248";
        $text = str_replace(["\r", "\n"], '', $text);
        $countryArray = explode('*', $text);
        //throw new Exception(print_r($countryArray, true));
        foreach ($countryArray as $detail):
            $countryDetailArray = explode('|', $detail);
            $countryFullName = $countryDetailArray[0];
            $shortname = strtolower($countryDetailArray[1]);
            $country = Country::find()->where(["countryName" => $countryFullName])->one();
            if (isset($country) && !empty($country)) {
                $saveName = 'images/flag/svg/' . $shortname . '.svg';
                $country->flag = $saveName;
                $country->save(false);
            }
        endforeach;
