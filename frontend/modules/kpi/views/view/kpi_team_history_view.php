...
<div class="col-12 ligth-gray-box">
    ...
    <div class="col-12 mt-15 pt-0" style="height:400px;overflow-y: auto;">
        <div class="row" style="--bs-gutter-x:0px;">
			...
			if (isset($kpiTeamHistory) && count($kpiTeamHistory) > 0) {
				...
				foreach ($kpiTeamHistory as $kgh):
					if (!isset($month[$kgh["month"]][$kgh["year"]])) {
						...
						if ($i == 0) {
							$currentSelect = $kgh['kpiTeamHistoryId'];
						}
			?>

            <div class="col-12 mt-5 small-content-pim pr-0 <?= $i == 0 ? 'selectedTeam' : 'bg-white' ?>"
                style="cursor: pointer;"
                onclick="javascript:showKpiTeamEmployeeUpdate(<?= $team['teamId'] ?>,<?= $kpiId ?>,'<?= $kgh['month'] ?>','<?= $kgh['year'] ?>',<?= $kgh['kpiTeamHistoryId'] ?>)"
                id="historyMonthYear-<?= $kgh['kpiTeamHistoryId'] ?>">
                <div class="row" style="--bs-gutter-x:0px;width:100%;">
                    <div class="col-4 align-content-center ">
                        ...
                        <div class="col-12 mt-5" style="font-size: 12px !important;font-weight: 400;color:#656565;">
                            <?= ModelMaster::engDate($kgh["fromDate"], 2) ?> -
                            <?= ModelMaster::engDate($kgh["toDate"], 2) ?>
                        </div>
                    </div>
                        ...
                        <div class="d-flex">
                            <div class="align-content-center " style="position: absolute;">
                                <svg viewBox="0 0 36 36" class="circular-chart-view dark-blue">
                                    <path class="circle-view-bg"
                                        d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                                    <path class="circle-view" id="KFI-progress-<?= $kgh['year'] ?>-<?= $kgh['month'] ?>"
                            ...
                            <div class="flex-grow-1">
                                <div class="text-b pr-0 text-end">
                                    <span
                                        style="font-size: 18px !important;font-weight: 500;"><?= ModelMaster::pimNumberFormat($kgh['result']) ?></span>
                                    <span style="font-size: 18px !important;font-weight: 700;color:#2580D3;"> /
                                        <?= ModelMaster::pimNumberFormat($kgh["target"]) ?></span>
                                </div>
                                <div class="text-end"
                                    style="font-size: 12px !important;font-weight: 400;color:#656565;">
                    ...
                    <div class="col-1 align-content-center pr-5" style="font-weight: 400;justify-items: end;">
                        <a class="<?= $i == 0 ? 'doubleplay-btn-blue' : 'doubleplay-btn' ?> flex-all-center"
                            id="btn-<?= $kgh['kpiTeamHistoryId'] ?>">
                            <img src="<?= Yii::$app->homeUrl ?>images/icons/pim/doubleplay-<?= $i == 0 ? 'white' : 'black' ?>.svg"
                                class="" id="img-<?= $kgh['kpiTeamHistoryId'] ?>" style="width:18px;height:18px;">
                        </a>
            ...
            <script>
            showFirstKpiTeamEmployeeUpdate(<?= $team['teamId'] ?>, <?= $kpiId ?>, '<?= $kgh['month'] ?>',
                '<?= $kgh['year'] ?>', <?= $kgh['kpiTeamHistoryId'] ?>);
            </script>
            <?php
						}
						$i++;
