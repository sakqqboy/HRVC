		...
		<div class="row">
			...
			?>
						<div class="col-12 mb-15">
							<div class="col-12 small-content-pim <?= $i == 0 ? 'selectedTeam' : 'bg-white' ?>" style="cursor: pointer;" onclick="javascript:showKpiTeamEmployeeUpdate(<?= $team['teamId'] ?>,<?= $kpiId ?>,'<?= $kgh['month'] ?>','<?= $kgh['year'] ?>',<?= $kgh['kpiTeamHistoryId'] ?>)" id="historyMonthYear-<?= $kgh['kpiTeamHistoryId'] ?>">
								<div class="row">
									<div class="col-5">
									...
									<div class="col-1 text-end pr-10 pl-0 pt-10 " style="font-weight: 400;">
										<a class="<?= $i == 0 ? 'doubleplay-btn-blue' : 'doubleplay-btn' ?>" id="btn-<?= $kgh['kpiTeamHistoryId'] ?>">
											<img src="<?= Yii::$app->homeUrl ?>images/icons/pim/doubleplay-<?= $i == 0 ? 'white' : 'black' ?>.svg" class="" id="img-<?= $kgh['kpiTeamHistoryId'] ?>">
										</a>
						...
						if ($i == 0) { ?>
							...
							<script>
								showFirstKpiTeamEmployeeUpdate(<?= $team['teamId'] ?>, <?= $kpiId ?>, '<?= $kgh['month'] ?>', '<?= $kgh['year'] ?>', <?= $kgh['kpiTeamHistoryId'] ?>);
							</script>
			<?php
						}
						$i++;
