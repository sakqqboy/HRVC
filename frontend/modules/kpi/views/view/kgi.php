		...
		<thead>
			<tr class="pim-table-header text-center">
				...
				<th style="border-top-right-radius: 4px;border-bottom-right-radius: 4px;" class="text-end">
					<a class="btn-blue-sm font-size-12 text-center no-underline mr-5 pl-10 pr-10" id="saveRelateKgi" href="javascript:showEditRelateKgi(2,<?= $kpiId ?>)">
						<img src="<?= Yii::$app->homeUrl ?>images/icons/Settings/save.svg" alt="" class="mr-3" style="margin-top: -1px; width: 16px;"> Save
					</a>
					<a class="btn-red-sm font-size-12 text-center no-underline pl-10 pr-10" id="cancelRelateKgi" href="javascript:showEditRelateKgi(0,<?= $kpiId ?>)"><?= Yii::t('app', 'Cancel') ?></a>
				</th>
			</tr>
		</thead>
		<tbody id="kgi">
			...
			if (isset($kpiHasKgi) && count($kpiHasKgi) > 0) {
				foreach ($kpiHasKgi as $kgiId => $kgi):
			?>
					<tr height="10">
					</tr>
					<tr id="kgi-<?= $kgiId ?>" class="text-center pim-table-text related-table-background">
						<td class="text-start font-b pt-10 text-primary" style="border-top-left-radius: 3px;border-bottom-left-radius: 3px;letter-spacing:0.5px;">
							<div class="text-truncate" style="max-width:300px;" title="<?= $kgi["kgiName"] ?>">
								<input type="checkbox" id="check-relate-kgi" class="checkbox-sm mr-5" value="<?= $kgiId ?>" name="kgi" <?= isset($kpiHasKgi[$kgiId]) ? 'checked' : '' ?>>
								<?= $i ?>.
								<?= $kgi["kgiName"] ?>
							</div>
						</td>
							...
							<div class="col-12 info-assign" style="margin-top: -3px;">
								<div class="row" style="--bs-gutter-x:0px;">
									...
									<div class="col-4  text-end pl-0 pr-5" style="align-content:center;">
										<img src="<?= Yii::$app->homeUrl ?>images/icons/Settings/down-darkblue.png" style="width:10px;height:7px;cursor:pointer;" onclick="javascript:showTeamKgi(<?= $kgiId ?>,1)" id="show-<?= $kgiId ?>">
										<img src="<?= Yii::$app->homeUrl ?>images/icons/Settings/up-darkblue.png" style="display: none;width:10px;height:7px;cursor:pointer;" onclick="javascript:showTeamKgi(<?= $kgiId ?>,0)" id="hide-<?= $kgiId ?>">
									</div>
								</div>
							</div>
			...
			foreach ($kgis["data"] as $kgiId => $kgi):
				...
				?>
					...
					<tr id="kgi-<?= $kgiId ?>" class="text-center pim-table-text related-table-background">
						<td class="text-start font-b pt-10 <?= isset($kpiHasKgi[$kgiId]) ? 'text-primary' : '' ?>" style="border-top-left-radius: 3px;border-bottom-left-radius: 3px;letter-spacing:0.5px;">
							<div class="text-truncate" style="max-width:300px;" title="<?= $kgi["kgiName"] ?>">
								<input type="checkbox" id="check-relate-kgi" class="checkbox-sm mr-5" value="<?= $kgiId ?>" name="kgi" <?= isset($kpiHasKgi[$kgiId]) ? 'checked' : '' ?>>
								<?= $i ?>.
								<?= $kgi["kgiName"] ?>
