				...
				<thead>
					<tr class="pim-table-header text-center">
						...
						<th style="border-top-right-radius: 4px;border-bottom-right-radius: 4px;">
							<a class="btn-blue-sm font-size-12 text-center no-underline" id="editRelateKgi" style="padding-left: 10px;padding-right:10px;display:<?= count($kfiHasKgi) == 0 ? 'none' : '' ?>" href="javascript:showEditRelateKgi(1,<?= $kfiId ?>)">
								<img src="<?= Yii::$app->homeUrl ?>images/icons/Settings/edit.svg" alt="" class="pim-icon mr-3" style="margin-top: -1px;">Add/Remove
							</a>
							<a class="btn-blue-sm font-size-12 text-center no-underline  mr-5 pl-10 pr-10" id="saveRelateKgi" style="display:none;" href="javascript:showEditRelateKgi(2,<?= $kfiId ?>)">
								<img src="<?= Yii::$app->homeUrl ?>images/icons/Settings/save.svg" alt="" class="pim-icon mr-3" style="margin-top: -1px;"> Save
							</a>
							<a class="btn-red-sm font-size-12 text-center no-underline pl-10 pr-10" id="cancelRelateKgi" style="display:none;" href="javascript:showEditRelateKgi(0,<?= $kfiId ?>)">Cancel</a>
						</th>
					</tr>
				</thead>
				<tbody id="kfiHasKgi">
					...
					?>
						...
						<tr id="kgi-<?= $kgiId ?>" class="text-center pim-table-text related-table-background">
							<td class="text-start font-b pt-10" style="border-top-left-radius: 3px;border-bottom-left-radius: 3px;letter-spacing:0.5px;">
								<input type="checkbox" id="check-relate-kgi" class="checkbox-sm mr-5" style="display:none;" value="<?= $kgiId ?>" name="kgi" <?= isset($ghp[$kgiId]) ? 'checked' : '' ?>>
								<?= $i ?>.
								<?= $kgi["kgiName"] ?>
							</td>
							<td><?= $kgi["month"] ?></td>
		...
		?>
			<div class="col-12 on-data-box mt-10 text-center">
				...
				<div class="col-12 font-size-16 font-b mt-5">
					Looks like there are no KGIs linked to this component yet.
				</div>
				<div class="col-12 mt-10 text-secondary">
					Click “Add KGI” to associate relevant metrics and track performance effectively.
				</div>
