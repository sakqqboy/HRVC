...
$current_year = mysqli_real_escape_string($connection, $_POST['start_year']);
$startYear = $current_year - 1 ; // start year is last year
...
$axis_X = array(); // Initialize array for X-axis

for ($j = 0; $j < 3; $j++) { // Outer loop for three years
    for ($i = 0; $i < 12; $i++) { // Inner loop for twelve months
        ...
        // Push the formatted date into the $axis_X array
        $axis_X[] = "$currentYear-$currentMonth";
    ...
    $startYear++; // Increment start year for the next iteration
}
