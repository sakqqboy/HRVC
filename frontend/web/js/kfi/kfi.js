...
function branchMultiDepartment() {
     ...
     if (multiBranch.length > 0) {
          ...
          $("#branch-selected-count").removeClass("cycle-current-gray").addClass("cycle-current-white");
          ...
          $('input[id="multi-check"]').each(function () {});
          $("#branch-selected-count").removeClass("cycle-current-white").addClass("cycle-current-gray");
     ...
     $("#branch-selected-message").text(selectedBranchCount > 0 ? `` : "No Branches are Selected Yet");
...
function departmentMultiTeam(branchId) {
     ...
     if (multiDepartment.length > 0) {
          ...
          $("#image-departments #department-selected-count").removeClass("cycle-current-gray").addClass("cycle-current-white");
     ...
     if (selectedDepartmentCount == 0) {
          $("#department-selected-count").text("00");
     } else {
          $("#department-selected-count").text(selectedDepartmentCount.toString());
     }
     $("#department-selected-message").text(selectedDepartmentCount > 0 ? `` : "No Departments are Selected Yet");
...
function multiTeam(departmentId) {
     ...
     if (totalChecked > 0) {
          ...
          $("#image-team #team-selected-count").removeClass("cycle-current-gray").addClass("cycle-current-white");
          ...
          // ปรับสไตล์ข้อความ
          $("#multi-team-text")
               .html(totalChecked + " Teams Selected")
               .css({
                    color: "#30313D",
                    "font-size": "14px",
                    "font-weight": "500",
          ...
          $("#team-selected-count").removeClass("cycle-current-white").addClass("cycle-current-gray");
     ...
     $("#team-selected-message").text(selectedTeamCount > 0 ? `` : "No Teams are Selected Yet");
}
...
function departmentMultiTeamUpdateKfi(branchId) {
     ...
     if (multiDepartment.length > 0) {
          $("#department-selected-count").text(multiDepartment.length);
     ...
     $("#department-selected-message").text(multiDepartment.length > 0 ? `` : "No Departments are Selected Yet");
}
...
function multiTeamUpdate(departmentId) {
     ...
     if (totalChecked > 0) {
          ...
          $("#team-selected-count").html(totalChecked);
          if (totalChecked == 1) {
               $("#img-team-1").find("img").attr("src", deptImageSrc);
               $("#img-team-1").removeClass("cycle-current-gray").addClass("cycle-current-team");
          }
     ...
     if ($("#multi-check-all-team-" + departmentId + "-update").prop("checked") == true) {
          $('input[id="multi-check-team-' + departmentId + '-update"]').each(function () {
               ...
               if (totalChecked > 0) {
                    ...
                    $("#image-team .cycle-current-gray").removeClass("cycle-current-gray").addClass("cycle-current").find("img").attr("src", deptImageSrc);

                    if (totalChecked < 3) {
                         $("#image-team .cycle-current").slice(totalChecked, 3).removeClass("cycle-current").addClass("cycle-current-gray").find("img").attr("src", deptBlackImageSrc);
                    }
...
function selectUnit(currentUnit) {
     ...
     if (previous != "") {
          ...
          $("#unit-" + previous)
               .removeClass("unit-active")
               .addClass("unit-inactive");
     }

     $("#currentUnit").val(currentUnit);
     $("#unit-" + currentUnit)
          .removeClass("unit-inactive")
...
function toggleCalendar(triggerId, calendarId) {
     ...
     if (trigger && calendarPopup) {
          trigger.addEventListener("click", function () {
               calendarPopup.style.display = calendarPopup.style.display === "none" || calendarPopup.style.display === "" ? "flex" : "none";
          });
     }
}
...
document.addEventListener("click", function (event) {
     const calendars = [
          {
               buttonId: "multi-due-term",
               calendarId: "calendar-due-term",
          },
          {
               buttonId: "multi-due-term-pim",
               calendarId: "calendar-due-term",
          },
          {
               buttonId: "multi-due-update",
               calendarId: "calendar-due-update",
          },
     ];
...
document.addEventListener("DOMContentLoaded", function () {
     ...
     if (page) {
          ...
          if (month && year) {
               document.getElementById("multi-month-year-text").innerHTML = getMonthName(month) + ", " + year;
               $("#multi-month-year-text").css({
                    color: "var(--HRVC---Text-Black, #30313D)",
               });
          ...
          if (startDate && endDate) {
               ...
               if (page == "pim") {
                    // document.getElementById("multi-due-term-text").innerHTML = newText + duterm + icon;
                    document.getElementById("multi-due-term-text").innerHTML = duterm;
                    ...
                    document.getElementById("multi-due-term").innerHTML = `${startDate} - ${endDate} <i class="fa fa-angle-down pull-right mt-5" aria-hidden="true"></i>`;
                    $("#calendar-dueterm").html(images1 + images2 + images3);
               }

               // เปลี่ยนสไตล์ข้อความ
               $("#multi-due-term-text").css({
                    color: "var(--HRVC---Text-Black, #30313D)",
               });
          ...
          if (nextCheckDate) {
               ...
               if (page == "pim") {
                    ...
                    document.getElementById("multi-due-update-text").innerHTML = nextDate;
                    ...
                    document.getElementById("multi-due-update").innerHTML = `${nextCheckDate} <i class="fa fa-angle-down pull-right mt-5" aria-hidden="true"></i>`;
               }

               // เปลี่ยนสไตล์ข้อความใน #multi-due-update
               $("#multi-due-update-text").css({
                    color: "var(--HRVC---Text-Black, #30313D)",
               });
...
function validateFormKfi(acType) {
     ...
     if (multiBranch.length == 0) {
          ...
          $("#branch-selected-message").show();
          $("#branch-selected-message").addClass("text-danger");
          return false;
     } else if (multiDepartment.length == 0) {
          $("#multi-department").addClass("not-valid");
          $("#department-selected-message").addClass("text-danger");
          return false;
...
function updateLastUpdateDate(dateStr) {
     ...
     if (dateStr) {
          ...
          if (page == "pim") {
               ...
               document.getElementById("multi-due-update-text").innerHTML = dateStr;
               const images1 = '<img src="' + $url + "image/calendar-blue.svg" + '" alt="from" class="calendar-due-image">';
               const images2 = '<img src="' + $url + "image/weld.svg" + '" alt="from" class="calendar-due-image">';
               const images3 = '<img src="' + $url + "image/calendar-blue.svg" + '" alt="from" class="calendar-due-image">';
               $("#calendar-dueterm-update").css("backgroundColor", "#D7EBFF");
...
function updateSelectedCount() {
     ...
     $("#selected-message").text(selectedCount > 0 ? `${selectedCount} Branch(es) Selected` : "No Branches are Selected Yet");
}
...
function viewTabKfi(kfiHistoryId, tabId, role = null) {
     ...
     var kfiId = $("#kfiId").val();
     $("#tab-" + currentTabId)
          .removeClass("view-tab-active")
          .addClass("view-tab");
     $("#tab-" + tabId)
          .removeClass("view-tab")
          .addClass("view-tab-active");
