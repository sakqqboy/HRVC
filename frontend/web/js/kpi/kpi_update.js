    ...
    for (a = 0; a < multiBranch.length; a++) {
      ...
      if (checkedDepartment == 0) {
        ...
        for (c = 0; c < multiDepartment.length; c++) {
          var multiTeam = [];
          var d = 0;
          $("#multi-check-team" + multiDepartment[d] + "-update:checked").each(
            function () {
          ...
          if (checkedDepartment == 0) {
            alert(
              "Please select at least 1 team for each selected department.",
            );
            isError++;
            return false;
          }
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.status) {
        ...
        if (data.lastest != 0) {
          $("#kpi-solution-" + data.lastest).addClass(
...
function departmentMultiTeamUpdateKpi(branchId) {
  ...
  if (multiDepartment.length > 0) {
    ...
    $("#multi-department-text").html(
      multiDepartment.length + ` Departments Selected`,
    );
    ...
    $("#image-departments .cycle-current img").attr(
      "src",
      $url + "image/departments-black.svg",
    );
    $("#image-departments .cycle-current")
      .slice(0, 3)
      .removeClass("cycle-current")
  ...
  $("#department-selected-message").text(
    selectedDepartmentCount > 0 ? `` : "No Departments are Selected Yet",
  );
...
function branchMultiDepartmentUpdateKpi() {
  ...
  $("#img-branch-1")
    .removeClass("cycle-current-branch")
    .addClass("cycle-current-gray");
  $("#img-branch-2")
    .removeClass("cycle-current-branch")
    .addClass("cycle-current-gray");
  ...
  if (multiBranch.length > 0) {
    $("#branch-selected-count")
      .removeClass("cycle-current-gray")
      .addClass("cycle-current-white")
      .html(multiBranch.length);
    $("#branch-selected-message").html(``);
    if (multiBranch.length == 1) {
      $("#img-branch-1")
        .removeClass("cycle-current-gray")
        .addClass("cycle-current-branch");
    }
    ...
    $("#image-branches #branch-selected-count")
      .removeClass("cycle-current-white")
      .addClass("cycle-current-gray");
...
function loadDepartmentKpi() {
  ...
  if (parseInt(totalDepartment) > 0) {
    ...
    if (totalDepartment == 1) {
      ...
      $("#img-department-1")
        .removeClass("cycle-current-gray")
        .addClass("cycle-current-department");
    }
...
function loadTeamKpi() {
  ...
  $("#img-team-1")
    .removeClass("cycle-current-team")
    .addClass("cycle-current-gray")
    .find("img")
    .attr("src", deptblackImageSrc);
  $("#img-team-2")
  ...
  if (multiTeam.length > 0) {
    ...
    // ปรับสไตล์ข้อความ
    $("#multi-team-text")
      .html(multiTeam.length + " Teams Selected")
      .css({
        color: "#30313D",
        "font-size": "14px",
        "font-weight": "500",
...
function assignKpiBranch(kpiId, branchId) {
  var checked = 0;
  if (
    $("#assign-branch-kpi-" + kpiId + "-" + branchId).prop("checked") == true
  ) {
    checked = 1;
  }
