...
function companyMultiBrach() {
  ...
  var companyId =
    acType == "update" ? $("#companyId").val() : $("#companyId").val();
  var kgiId = $("#kgiId").val();
...
function totalTeamUpdate(departmentId) {
  ...
  var data = [];
  var i = 0;
  $('input[id="multi-check-team-' + departmentId + '-update"]').each(
    function () {
      data[i] = $(this).val();
      i++;
    },
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.status) {
        ...
        $("#lastest-solution-" + data.kgiId).html(data.solution);
        if (data.lastest != 0) {
          $("#kgi-solution-" + data.lastest).addClass(
...
function assignKgiBranch(kgiId, branchId) {
  var checked = 0;
  if (
    $("#assign-branch-kgi-" + kgiId + "-" + branchId).prop("checked") == true
  ) {
    checked = 1;
  }
...
function kgiUpdateHistory(kgiId) {
  ...
  $.ajax({
    ...
    dataType: "json",
    url: url,
    data: { kgiId: kgiId, month: month, year: year },
    success: function (data) {
