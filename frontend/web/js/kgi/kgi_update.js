...
function departmentMultiTeamUpdate(branchId) {
  ...
  $("#img-department-1")
    .removeClass("cycle-current-department")
    .addClass("cycle-current-gray")
    .find("img")
    .attr("src", deptblackImageSrc);
  $("#img-department-2")
    ...
    for (a = 0; a < multiBranch.length; a++) {
      ...
      if (checkedDepartment == 0) {
        ...
        if (branchDepartment > 0) {
          isError++;
          alert(
            "Please select at least 1 department for each selected branch.",
          );
          return false;
        }
      } else {
...
function approveRequestKgiEmployee(kgiEmployeeHistoryId, approve) {
  ...
  if (confirm(confirmText)) {
    $.ajax({
      type: "POST",
      dataType: "json",
      url: $url + "kgi/management/approve-kgi-employee-request",
      data: {
      ...
      success: function (data) {
        if (data.status) {
          ...
          window.location.href =
            $url + "kgi/management/wait-approve-kgi-personal";
        } else {
